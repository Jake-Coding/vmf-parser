...
class VMF(ve.VMFElement):
    ...
    textures_to_change: dict[str, dict[str, str]] = {
        # {entity name (lowercase) : {old_texture (UPPERCASE) : new_texture}} make a class?
    ...
    def change_texture_to_momentum(entity: ve.VMFElement) -> None:
        ...
        :rtype: None
        """
        if entity.first_layer_has("solid") and (
                ent_type := entity.get_subproperties_by_name("classname")[
                    0].get_value().lower()) in VMF.textures_to_change:
    ...
    def tf2_remove_class_attrs(self, class_n: int | str, all_except_one: bool = False) -> None:
        ...
        while i < len(self.elements["entities"]):
            ...
            if entity.first_layer_has("classname", "filter_tf_class") and entity.first_layer_has("targetname", None,
                                                                                                 False):
        ...
        names_to_remove: list[str] = []  # if a filter_multi is deleted, we need to remove all references to it
        i = 0
        while i < len(self.elements["entities"]):
            entity = self.elements["entities"][i]
            ...
            if entity.first_layer_has("classname", "filter_multi"):
                ...
                for num in range(10):
                    if (entity.first_layer_has(f"Filter{num + 1}")):
                        if entity.get_subproperties_by_name(f"Filter{num + 1}")[0].get_value() in class_only_trigger_names:
                            if and_flag:
                                ...
                                entity.delete_property(f"Filter{num + 1}")
                if not and_flag:
    ...
    def tf2_simplify_class_attrs(self, class_n: int | str) -> None:
        ...
        while i < len(self.elements["entities"]):
            ...
            if entity.first_layer_has("classname", "filter_tf_class"):
                filter_entities_names.append(entity.get_subproperties_by_name("targetname", False)[0].get_value())
                self.elements["entities"].remove(entity)
                continue
            ...
            if entity.first_layer_has("classname", "filter_multi"):
                for num in range(10):
                    if entity.first_layer_has(f"Filter{str(num + 1).zfill(2)}"):
                        if entity.get_subproperties_by_name(f"Filter{str(num + 1).zfill(2)}")[0].get_value() in filter_entities_names:
                            entity.delete_property(f"Filter{str(num + 1).zfill(2)}")
    ...
    def tf2_to_momentum(self, for_class_num: int | str) -> None:
        ...
        while i < len(self.elements["entities"]):
            ...
            if entity.first_layer_has("classname", "func_regenerate"):
                ...
                if connections and connections.first_layer_has("OnStartTouch", None, False):
                    ...
                    if len(connections.get_properties()) == 0:  # if the trigger_multiple has no more properties, remove it
                        self.elements["entities"].remove(entity)
                        continue
                ...
                if not connections.first_layer_has("OnDamaged", None, False):  # if the button doesn't trigger onDamaged
                    for on_press in connections.get_subproperties_by_name("OnPressed", None, False):
                ...
                if not entity.first_layer_has("launchtarget"):
                    ...
                    if direction[0] % 360 == 270 and direction[1] % 360 == 0 and direction[2] % 360 == 0:
                        player_speed.set_value(
                            str(float(player_speed.get_value()) * 1.5))  # multiply the velocity by 1.5
        ...
        self.tf2_simplify_class_attrs(
            for_class_num)  # remove all filters for this class, and all filternames for this class
